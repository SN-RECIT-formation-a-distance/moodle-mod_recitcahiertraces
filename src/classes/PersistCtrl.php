        ...
        try{
            ...
            foreach ($data as $collection){
                foreach ($collection as $note){
                    ...
                    $query = "select t1.id, t3.course FROM {recitct_notes} t1 
                    inner join {recitct_groups} t2 on t1.gid = t2.id
                    inner join {recitcahiertraces} t3 on t2.ct_id = t3.id
                    where t1.intcode = ? and t3.course = (select course from {course_modules} where id = ?)
                    order by id desc limit 1";

                    $intCode = $this->getRecordsSQL($query, [$note->intCode, $mCmId]);
                    ...
                    $result = $this->getRecordsSQL("select t1.id g_id, t1.name group_name, t1.slot group_slot, t1.ct_id ct_id FROM {recitct_groups} t1
                    where t1.name = ? and t1.ct_id = ? order by name asc limit 1", [$obj->group->name, $obj->group->ct->id]);
