...
function Create-Zipfile($zipName, $pluginName, $pluginVersion){
	...
	& "c:\Program Files\7-Zip\7z.exe" a -mx "$zipFile" "src\*"-mx0 -xr!"src\react\.cache" "src\*"-mx0 -xr!"src\react\.parcel-cache" -xr!"src\react\node_modules" -xr!"src\react\.babelrc" -xr!"src\react\package-lock.json" -xr!"*.map"

	#set the plugin name
	& "c:\Program Files\7-Zip\7z.exe" rn "$zipFile" "src\" "$pluginName\"
}

$zipName="mod_recitcahiertraces"
